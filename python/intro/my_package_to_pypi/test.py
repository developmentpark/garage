...
from stats_gaussian_binomial import Gaussian
from stats_gaussian_binomial import Binomial

class TestGaussianClass(unittest.TestCase):
    def setUp(self):
        self.gaussian = Gaussian(25, 2)
        self.gaussian_data = Gaussian()
        self.gaussian_data.read_data_file('numbers.txt')
    ...
    def test_readdata(self):
        self.assertEqual(self.gaussian_data.data,\
         [1, 3, 99, 100, 120, 32, 330, 23, 76, 44, 31], 'data not read in correctly')

    def test_meancalculation(self):
        self.assertEqual(self.gaussian_data.calculate_mean(),\
         sum(self.gaussian_data.data) / float(len(self.gaussian_data.data)), 'calculated mean not as expected')

    def test_stdevcalculation(self):
        self.assertEqual(round(self.gaussian_data.calculate_stdev(), 2), 92.87, 'sample standard deviation incorrect')
        self.assertEqual(round(self.gaussian_data.calculate_stdev(0), 2), 88.55, 'population standard deviation incorrect')

    def test_pdf(self):
        self.assertEqual(round(self.gaussian.pdf(25), 5), 0.19947,\
         'pdf function does not give expected result') 
        self.gaussian_data.calculate_mean()
        self.gaussian_data.calculate_stdev()
        self.assertEqual(round(self.gaussian_data.pdf(75), 5), 0.00429,\
